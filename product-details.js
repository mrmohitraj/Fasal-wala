...
document.addEventListener('DOMContentLoaded', () => {
  ...
  document.getElementById('offerBtn').addEventListener('click', ()=> {
    ...
    const msg = `Hi, I'm ${cu.name} from Mr Fasal Wala. I'm interested in your listing "${product.name}" (₹${product.price}/${product.unit}). I'd like to negotiate the price for ${qty} ${product.unit}. Could we discuss?`;
    window.location.href = `https://wa.me/${number}?text=${encodeURIComponent(msg)}`;
  });

  document.getElementById('reportBtn').addEventListener('click', ()=> {
    const number = OWNER_WHATSAPP_NUMBER;
    const msg = `⚠️ Reporting a listing on Mr Fasal Wala\nProduct: ${product.name} (ID: ${product.id})\nSeller: ${product.seller}\nReason: `;
    window.location.href = `https://wa.me/${number}?text=${encodeURIComponent(msg)}`;
  });
});
